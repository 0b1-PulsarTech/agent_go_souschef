...
func TestChangedDetectsDirty(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "dirty.go"), []byte("package x"), 0o644); err != nil {
	...
	if got == "No matching changes." || got == "No git diff available." {
