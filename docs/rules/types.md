...
`map[string]any` is acceptable only at the I/O boundary — JSON decode buffer, generic metadata
bag attached to an `apperr.Error`, etc. Translate to a typed value as soon as possible.
...
```go
// internal/domain/usecases/secretfriend/secretfriend.go
...
The `mysqlrepo.SecretFriendRepo` struct satisfies `secretfriend.Repository` structurally. The
consumer doesn't need to import the implementor; the implementor doesn't need to know about the
consumer.
...
The canonical example in this repo is `libs/tereckernel/confloader.BindField[T]`, which collapses
what would otherwise be one binder per primitive type into a single parametric helper:

```go
func BindField[T any](ptr *T, field string, parser func(string) T) Binder { ... }
```

Other shapes that benefit from generics:

- Container helpers (`slices.Map`, `slices.Filter`, `set.New[T]`).
- Repository helpers that wrap `dbgen` calls returning `[]Row` and translate to a domain slice.
