...
- Lowercase, single word, no underscores, no `mixedCaps`.
  - Good: `secretfriend`, `confloader`, `webwrap`, `mysqlrepo`, `webport`.
  - Bad: `secret_friend`, `secretFriendPackage`, `myutils`, `helpers`.
- The package name is the user's first contact with the API; pick one that reads well at the call
  site (`secretfriend.New(...)`, not `secretFriendPkg.NewSecretFriend(...)`).
- Avoid stuttering: a function exported from package `parsers` is `parsers.Parse`, not
  `parsers.ParseParser`. Similarly `apperr.New`, not `apperr.NewError`.
...
- Module path matches the import path: `github.com/0b1-PulsarTech/terectek_comms/<dir>`.
...
The platform domain language is **English**. WhatsApp/Telegram/Instagram terms (template, session,
channel, contact, conversation, message) keep their canonical English names. Avoid coining
PT‑BR identifiers in Go code; reserve PT‑BR for user‑facing strings only.
