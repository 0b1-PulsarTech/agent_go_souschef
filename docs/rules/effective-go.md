...
- Do not stutter: `secretfriend.New`, not `secretfriend.NewSecretFriend`.
...
- Libraries never `panic`. Programmer‑error invariants use `panic` only in `cmd/<app>/main.go` boot
  paths where there is no recoverable state (e.g. failed DI registration).
